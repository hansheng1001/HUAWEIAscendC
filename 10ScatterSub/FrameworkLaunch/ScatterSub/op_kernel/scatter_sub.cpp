  ...
  __aicore__ inline void Compute(int32_t progress) {
    ...
    if (BUFFER_NUM == 1) {
      if (1 == this->updateTimesPerTensor) {
        ...
        if ((this->updateTimesPerTensor - 1) == IndexT) {
          if (2 != this->dataType) {
            Sub(yLocal, xLocal, uLocal, this->lastUpPerTensordataLen);
          } else {
            ...
            Sub(yHalf, xHalf, uHalf, this->lastUpPerTensordataLen);
