...
public:
  ...
  Init(GM_ADDR x, GM_ADDR y, GM_ADDR out, uint32_t reduction,
       uint32_t dataTotalLength, uint32_t blockLength, uint32_t tileNum,
        ...
        if (0 == ((this->lasttileLength * sizeof(DTYPE_X)) % 32)) {
          ...
          if (0 != dLen) {
            ...
            DataCopy(inLocal[this->tileLength],
                     yGm[progress * this->tileLength], dLen);
          }
    ...
    if (1 == BUFFER_NUM) {
      if (progress == this->tileNum - 1) {
        ...
        if ((1 == this->reduction) || (2 == this->reduction)) {
          ...
          // Duplicate(yLocal, DTYPE_X(0), this->tileLength);

          if (0 == ((this->lasttileLength * sizeof(DTYPE_X)) % 256)) {
            // 针对不是2**n时, ReduceSum有问题
            ReduceSum(xLocal, xLocal, yLocal, this->lasttileLength);
            this->sum += (float)xLocal.GetValue(0);
          } else {
            for (uint32_t i = 0; i < this->lasttileLength; i++) {
              this->sum += (float)xLocal.GetValue(i);
            }
          }
    ...
    if (0 == this->reduction) {
      ...
      if (BUFFER_NUM == 1) {
        if (progress == this->tileNum - 1) {
          ...
          if (0 == ((this->lasttileLength * sizeof(DTYPE_OUT)) % 32)) {
            ...
            uint32_t dLen = this->lasttileLength / (32 / sizeof(DTYPE_OUT)) *
                            (32 / sizeof(DTYPE_OUT));
            uint32_t restLen = this->lasttileLength % (32 / sizeof(DTYPE_OUT));

            if (0 != dLen) {
              DataCopy(outGm[progress * this->tileLength], outLocal, dLen);
            }

            for (uint32_t i = 0; i < restLen; i++) {
              outGm.SetValue(progress * this->tileLength + dLen + i,
                             outLocal.GetValue(dLen + i));
            }
          }
      ...
      if (progress == (this->tileNum * BUFFER_NUM - 1)) {
        ...
        outGm.SetValue(0, (this->sum / this->dataTotalLength));
...
mse_loss(GM_ADDR x, GM_ADDR y, GM_ADDR out, GM_ADDR workspace, GM_ADDR tiling) {
  ...
  op.Init(x, y, out, tiling_data.reduction, tiling_data.dataTotalLength,
