...
public:
  ...
  Init(GM_ADDR predict, GM_ADDR label, GM_ADDR x, GM_ADDR z, float cof,
    ...
    if (tilingKey == 1) {
      ...
      // zGm.SetGlobalBuffer(
      //     (__gm__ DTYPE_Z *)(z - (this->tileLength - this->lasttileLength) *
      //                                sizeof(DTYPE_Z)) +
      //         this->blockLength * GetBlockIdx(),
      //     this->blockLength);

      // zGm.SetGlobalBuffer(
      //     (__gm__ DTYPE_Z *)z + this->blockLength * GetBlockIdx(), 420);
    }
    ...
    if (BUFFER_NUM == 1) {
      if (progress == this->tileNum - 1) {
        ...
        if (progress == 0) {
          ...
          //            lGm[progress * this->tileLength], this->lasttileLength);
          //   DataCopy(inLocal[2 * this->tileLength],
          //            xGm[progress * this->tileLength], this->lasttileLength);
          ...
          //   if (0 != dLen) {
          //     DataCopy(inLocal[0], pGm[progress * this->tileLength], dLen);
          //     DataCopy(inLocal[this->tileLength],
          //              lGm[progress * this->tileLength], dLen);
          //     DataCopy(inLocal[2 * this->tileLength],
          //              xGm[progress * this->tileLength], dLen);
          //   }

          //   for (uint32_t i = 0; i < restLen; i++) {

          //     inLocal.SetValue(dLen + i, pGm.GetValue(dLen + i));
    ...
    if (BUFFER_NUM == 1) {
      if (progress == this->tileNum - 1) {
        ...
        if (progress == 0) {
          ...
          if (0 == ((this->lasttileLength * sizeof(DTYPE_Z)) % 32)) {
            // DataCopy(zGm[0], zLocal, this->tileLength);

            DataCopy(zGm[0], zLocal, this->lasttileLength);
          } else {
            ...
            uint32_t dLen = this->lasttileLength / (32 / sizeof(DTYPE_Z)) *
                            (32 / sizeof(DTYPE_Z));
            uint32_t restLen = this->lasttileLength % (32 / sizeof(DTYPE_Z));

            if (0 != dLen) {
              DataCopy(zGm[0], zLocal, dLen);
            }

            for (uint32_t i = 0; i < restLen; i++) {
              zGm.SetValue(dLen + i, zLocal.GetValue(dLen + i));
            }
          }
